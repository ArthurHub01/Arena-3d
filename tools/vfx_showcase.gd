...
func _wait_for_match() -> void:
	if not is_instance_valid(arena) or not is_instance_valid(arena.player_one) or not arena.player_one.match_active:
		await get_tree().create_timer(0.2).timeout
		_wait_for_match()
		return
	print("Match active, starting showcase.")
	_next_ability()
...
func _next_ability() -> void:
	...
	print("Firing ", tier, " ", ElementType.display_name(element), " (", ability.vfx_key, ")")
