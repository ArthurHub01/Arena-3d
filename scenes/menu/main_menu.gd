...
const CUSTOMIZE_SCENE_PATH := "res://scenes/menu/CustomizeMenu.tscn"
...
@onready var customize_button: Button = $CenterContainer/Columns/LeftPanel/LeftContent/CustomizeButton
...
func _check_cmdline_autoconnect() -> void:
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--element="):
			...
			match name:
				"water": NetworkState.selected_element = ElementType.Type.WATER
				"earth": NetworkState.selected_element = ElementType.Type.EARTH
				"air": NetworkState.selected_element = ElementType.Type.AIR
				"lightning": NetworkState.selected_element = ElementType.Type.LIGHTNING
		if arg.begins_with("--character="):
			NetworkState.selected_character_id = arg.split("=", true, 1)[1]
	for arg in OS.get_cmdline_user_args():
		if arg == "--auto-host":
			_on_host_pressed()
